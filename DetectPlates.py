...
def detectPlatesInScene(imgOriginalScene):
    ...
    imgGrayscaleScene, imgThreshScene = Preprocess.preprocess(imgOriginalScene) # tiền xử lý đưa ảnh sang thang màu xám
...
def findPossibleCharsInScene(imgThresh):
    ...
    imgContours, contours, npaHierarchy = cv2.findContours(imgThreshCopy, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)   #xác định các đường viền đã có
...
def extractPlate(imgOriginal, listOfMatchingChars):
    ...
    listOfMatchingChars.sort(key = lambda matchingChar: matchingChar.intCenterX)        # sắp xếp các kí tự từ trái sang phải
